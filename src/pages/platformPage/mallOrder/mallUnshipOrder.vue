<template>
	<div class="allOrder commodity" @click="closeSearch" >
		...
		<el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" :show-close="false" class="order" :title="type">
			...
			<el-button class="productSet float-r" v-if="type==='订单详情'&&orderLists[index].status===2" @click="setPro">商品发货</el-button>
			<!--.................主体内容....................-->
		...
		<div class="g-content">
			...
			<!--........................表格...............-->
			<bought :orderData="orderData" :orderMess="orderMess" @handleCurrent="handleCurrent" @showOrder="showOrder" 
				@showSetOrder="showSetOrder" :orderLists="orderLists" :isShopName="true"  v-loading="loading">
	...
	export default {
		...
		created() {
			...
			if(JSON.stringify(this.$route.query)!=="{}"){
				switch(true){
					case this.$route.query.name==="express":
						this.queryCon("express");
						break;
					case this.$route.query.name==="shop":
						this.queryCon("shop");
						break;
					case  this.$route.query.name==="self":
						this.queryCon("self");
						break;
				}
			};
		...
		methods:{
			...
			queryCon(data){
				...
				this.orderMess.search=Object.assign({},this.orderMess.search,{
					distribute_type:data
				});
				this.distribute_type.push(data);
			},
