<template>
  <div class="withdrawal-list-wrap">
    <div class="order-list-wrap">
      <el-table v-loading="Loading" class="withdrawal-list-wrap" :data="list" style="width: 100%">
        ...
        <el-table-column prop="address" label="操作" width="120">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status==2&&role=='shop'" @click="withdrawalConfirm(scope.row.tixian_id)" :loading="withdrawalClickId==scope.row.tixian_id"
              class="btn-withdrawal-confirm" type="primary" size="mini"> 确认收款</el-button>
            <p class="font-primary" @click="withdrawalDetailShow(scope.row.tixian_id)"> 查看记录 </p>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <el-dialog title="提现记录" :visible.sync="dialogVisible" :before-close="handleClose" class="dialog-wrap">
      <p class="panel-title">提现进度</p>
      <div v-loading="dialogDataLoading">
        <div v-if="!dialogDataLoading" class="dialog-content">
          <ul class="progess clearfix">
            ...
            <li class="detail-item item-active" v-if="detailForWithdrawal.status!==3">
              <div>
                <i class="icon-line"></i>
                <i class="icon-dot"></i>
              </div>
            ...
            <li :class="['detail-item',{'item-active':detailForWithdrawal.status%2==0}]" v-if="detailForWithdrawal.status!==3">
              <div>
                <i class="icon-line"></i>
                <i class="icon-dot"></i>
              </div>
            ...
            <li :class="['detail-item',{'item-active':detailForWithdrawal.status==4}]" v-if="detailForWithdrawal.status!==3">
              <div>
                <i class="icon-line"></i>
                <i class="icon-dot"></i>
              </div>
          ...
          <ul class="panel-detail">
            <li>
              <label>提现金额：</label>{{detailForWithdrawal.apply_money_yuan | money}}</li>
            <li>
              <label>提现手续费：</label>{{detailForWithdrawal.deduct_money_yuan | money}}</li>
            <li>
              <label>最终提现金额：</label>{{detailForWithdrawal.money_yuan | money}}</li>
            <li>
              <label>开户姓名：</label>{{detailForWithdrawal.open_name}}</li>
            <li>
              <label>开户银行：</label>{{detailForWithdrawal.open_bank}}</li>
            <li>
              <label>银行卡号：</label>{{detailForWithdrawal.open_number}}</li>
          </ul>
          <div v-if="detailForWithdrawal.status==3">
            <p class="panel-title">提现失败原因</p>
            <p class="remark">{{detailForWithdrawal.remark}}</p>
          </div>
          <p v-if="role=='mall'" class="panel-title">卖家信息</p>
          <ul v-if="role=='mall'" class="panel-detail">
            <li>
              <label>店铺名称：</label>{{detailForWithdrawal.shop_name }}</li>
            <li>
              <label>店
                <i></i>长：</label>{{detailForWithdrawal.shopkeeper_name}}</li>
            <li>
              <label>联系方式：</label>{{detailForWithdrawal.shopkeeper_phone}}</li>
          </ul>
        </div>
      </div>
      <span slot="footer" v-if="detailForWithdrawal.status==2&&role=='shop'" class="dialog-footer">
        <el-button type="primary" @click="withdrawalConfirm(detailForWithdrawal.tixian_id)" :loaging="submitDisable">
          确认收款
        </el-button>
      </span>
      <span slot="footer" v-if="detailForWithdrawal.status==1&&role=='mall'" class="dialog-footer">
        <el-button @click="mallRemittance(2)" :disabled="submitDisable" :loading="btnLoadingIndex==2" class="btn-agree" type="primary">
          汇款成功
        </el-button>
        <el-button @click="remittanceRejectConfirm" :disabled="submitDisable" :loading="btnLoadingIndex==3" class="btn-reject">
          汇款失败
        </el-button>
      </span>
    </el-dialog>
