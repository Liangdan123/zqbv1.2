...
<script>
	...
	export default{
		...
		components:{
			"orderDetailed":()=>import("@/components/platform/fund/orderDetailed")
		},
		data(){
			return{
		...
		methods:{
			...
			checkDetail(index){//查看明细按钮
				...
				this.name=this.list.data[index].contact_name+"的订单收入明细"
				console.log("this.list.data[index]：",this.list.data[index])
			},
			searchRole(){//角色搜索
