<template>
	<div class="orderDetai clearfix">
		<div class="plate mb-40">
			...
			<!--.............未付款时不显示..............-->
			<div class="small_plate" v-if="checkOrder.status===1||checkOrder.status===5||checkOrder.status===6?false:true">
				<span class="color-7F f14 display-in">配送方式：</span>
				<em class="color-3 f14 display em">
					{{checkOrder.distribute_type==="express"?"快递配送":checkOrder.distribute_type==="shop"?"门店配送":"到店自取"}}
				</em>
			</div>
			<!--.............未付款时不显示..............-->
				...
				<div class="progress_sec float-l mr_fu_12" v-if="checkOrder.status!==5&&checkOrder.status!==6">
					...
					<div class="progress_txt mr_fu_12">
						<span class="f14" :class="checkOrder.status===1?'color-6':'color-b'">
							{{checkOrder.distribute_type==="express"?"等待卖家发货":checkOrder.distribute_type==="shop"?"等待卖家送货":"等待卖家备货"}}
						</span><br>
				...
				<div class="progress_sec float-l mr_fu_12" v-if="checkOrder.status!==5&&checkOrder.status!==6">
					...
					<div class="progress_txt mr_fu_12">
						<span class="f14" :class="checkOrder.status!==1&&checkOrder.status!==2?'color-b':'color-6'">
							{{checkOrder.distribute_type==="express"?"等待买家收货":checkOrder.distribute_type==="shop"?"等待买家收货":"等待买家提货"}}
						</span><br>
		...
		<div class="plate mb-40">
			...
			<!--.................订单详情表单..........-->
			<div v-if="type==='订单详情'">
				<productTable :productMess="checkOrder.order_products"></productTable>
			</div>
			<!--.................发货表单..........-->
			<div v-if="type==='发货'" class="clearfix">
				<el-table :data="checkOrder.order_products" style="width: 100%" @selection-change="handleSelectionChange">
    				...
    				<el-table-column label="配送方式">
     					<template slot-scope="scope">
     						<div>{{scope.row.distribute_type==="express"?"快递配送":scope.row.distribute_type==="shop"?"门店配送":"到店自取"}}</div>     							     						
     					</template>
     				</el-table-column>
				</el-table>
				<span class="f12 color-7F mt-10 display-in">注：若选择部分商品进行发货，订单将被拆分</span><br>			
			</div>
		</div>
		<!--.................发货.................-->
		<div class="plate clearfix " >
			<div  class="float-l" v-if="checkOrder.distribute_type==='express'&&type==='发货'">
				<div class="float-l">
					<span class="f14 color-3">快递公司：</span>
					<el-select v-model="express_code" placeholder="请选择快递公司">
					    <el-option v-for="item in expressMess" :key="item.express_code" :label="item.express_name" :value="item.express_code">
					      					     
					    </el-option>
					</el-select>
				</div>
				<div class="float-l ml-20">						
					<el-input v-model.trim="checkOrder.express_number" class="expressNum"></el-input>
					<span class="float-l expressNum f14  color-3" >快递单号：</span>
				</div>
			</div>	
			<div class="float-r btn" v-if="type==='发货'">
				<el-button class="store-button1" @click="sureSetPro" v-if="checkOrder.distribute_type==='express'||checkOrder.distribute_type==='shop'">确定发货</el-button>
				<el-button class="store-button1" @click="sureSetPro" v-if="checkOrder.distribute_type==='self'">确定备货</el-button>
				<el-button class="store-button2" @click="cancelPro">取消</el-button>
			</div>
		</div>

		<!--.................物流信息.................-->
		<div  class="plat" v-if="type==='订单详情'&&(checkOrder.status===3||checkOrder.status===4)&&checkOrder.distribute_type==='express'">
			<p class="color-3 f16 font-b mb-20">物流信息</p>
			<LogisticsInfo :split_order_id="checkOrder.split_order_id" :shipping_time="checkOrder.shipping_time" :logisticsInfo="logisticsInfo">
...
<script>
	import { getExpress,setProduct} from "@/api/order"
...
//			判断是发货还是订单详情弹框
		...
		methods:{
			...
			//确定发货
			sureSetPro(){
				if(this.checkOrder.distribute_type==='express'&&this.type==='发货'){
					if(this.express_code===""){
						this.PromptMess("请选择快递公司","warning");
						return
					};
					if(this.checkOrder.express_number===null){
						this.PromptMess("请填写快递单号","warning");
						return
					}
				};
				//点击时候订单的长度
				...
				//选择快递公司时传给后台的值
				if(this.express_code!==""){
					setProMess=Object.assign({},setProMess,{
						express_code:this.express_code
					})
				};
				//填了快递单号传的值
				if(this.checkOrder.express_number!==null&&this.checkOrder.express_number!==""){
					setProMess=Object.assign({},setProMess,{
						express_number:this.checkOrder.express_number
					})
				};
				//卖家发货API
