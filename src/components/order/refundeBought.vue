<template >
	<div class="refundBought">
		...
		<!--...................物流弹框...........-->
		<el-dialog title="物流信息" :visible.sync="LogisticsVis">
			<LogisticsInfo :refund_order_id="refund_order_id" :shipping_time="shipping_time" :refundInfo="refundInfo">				
			</LogisticsInfo>
		</el-dialog>
		<!--...................搜素...........-->
			...
			<template>
				<div>				
					<div class="condition clearfix">
						<span class="f12 color-3 float-l">退款方式：</span>
						<div class="float-l">
							<label class="display-b store_label">
					        	<input type="checkbox" name="refund" class="display-n" v-model="refund" @change="distri('1')">
					        	<em></em>	
					        	仅退款
					        </label>
					        <label class="display-b store_label">
					        	<input type="checkbox" name="refundPro" class="display-n" v-model="refundPro" @change="distri('2')">
					        	<em></em>	
					        	退货退款
					        </label>
						</div>
					</div>
					<div class="condition clearfix mt-10" v-if="this.orderMess.search.type===2">
						<span class="f12 color-3 float-l">退款状态：</span>
						<div class="float-l">
							<label class="display-b store_label">
					        	<input type="checkbox" name="shipment" class="display-n" v-model="shipment" @change="setStatus('1')">
					        	<em></em>	
					        	等待买家发货
					        </label>
					        <label class="display-b store_label">
					        	<input type="checkbox" name="receipt" class="display-n" v-model="receipt" @change="setStatus('2')">
					        	<em></em>	
					        	等待商家收货
					        </label>
					        <label class="display-b store_label">
					        	<input type="checkbox" name="account" class="display-n" v-model="account" @change="setStatus('3')">
					        	<em></em>	
					        	等待退款到账
					        </label>
						</div>
			...
			<div class="bought" v-for="(item,index) in orderData.data">
				...
				<div class="clearfix pos-r">				
					...
					<ul class="clearfix pos-a ul_right">						
						...
						<li>
							<div class="v_center">
								{{item.refund_type===1?"仅退款":"退货退款"}}
							</div>
						</li>
						<li>
							<div class="v_center text-c" v-if="item.refund_type===1">
								{{item.refund_status===1?"等待商家同意":item.refund_status===2?"买家撤销退款":item.refund_status===3?
								"商家已拒绝":item.refund_status===4?"等待退款到账":item.refund_status===5?"已退款":"退款失败"}}																							
							</div>						
							<div  class="v_center text-c" v-if="item.refund_type===2">
								{{item.refund_status===11?"等待商家同意":item.refund_status===12?"买家撤销退款":item.refund_status===13?"商家已拒绝":item.refund_status===14?
								"等待买家发货":item.refund_status===15?"等待商家收货":item.refund_status===16?"等待退款到账":item.refund_status===17?"已退款":"退款失败"}}									
							</div>
						</li>
						<li>
							<div class="v_center">
						...
						<li>
							<div class="v_center btn">
								<div class="clearfix mb-10" v-if="item.refund_status===1||item.refund_status===11">									
								...
								<el-button class="store-button3 mb-5 display-b" @click="refConfirm(item)"
									v-if="item.refund_status===16||item.refund_status===14||item.refund_status===15">
									确认收货
								</el-button>
								<span class=" color-b cursor display-b mb-5" v-if="item.refund_status===15" @click="checkLogistics(item)">
									查看物流
								</span>
...
<script>
	...
	import {refundAgree,refundDisagree,refundConfirm,refundLogMess} from "@/api/order"
		...
		components: {
  		...
  		methods:{
  			...
  			checkOrder(data){
				...
				if(this.orderData.data[data].refund_status===15){
					let refund_order_id=this.orderData.data[data].refund_order_id;	
					this.checkRefund(refund_order_id)
				};
  			},
  			 //筛选订单的确定按钮
  			searchMthod(data){ 	
				this.$emit("searchMthod",data);
  			},
  			//商品名称搜索
  			inputSearch(data){
  			...
  			//清空条件
  			emptyMthod(){
				var arr=["refundPro","refund","shipment","receipt","account"];
				arr.map(item=>this[item]=false)
  				
  				this.refund_type=[];
  				this.wait=[];
  				if(this.orderMess.search.refund_type!==undefined){
					delete this.orderMess.search.refund_type
				};
				if(this.orderMess.search.status){
					delete this.orderMess.search.status
				}
  				//去父集调接口 				
			...
			//退款状态选择条件
			setStatus(data){
				setUnion(data,this.wait);
				if(this.wait.length){
					this.lastWait=this.wait.join("|");
					this.$set(this.orderMess.search,"status",this.lastWait);	
			...
			checkRefund(data){
			    ...
			    .then((data)=>{
			    	...
			    	this.$emit("showlogist",this.refundInfo);
			    })
			    .catch(({response: {data}})=>{
			    	this.$message.error(data.errorcmt);
			    })
