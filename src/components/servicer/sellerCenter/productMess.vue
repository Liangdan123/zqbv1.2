	...
	<div class="productMess">
		<div class="g-content">
			...
			ref="pulishForm">
				...
				<el-form-item
					prop="product_detail_arr">
					<CommodityDetails
						:CommodityDetails="pulishForm.product_detail_arr">
			...
			<div class="parentMess" 
				v-if="editShow==='编辑商品'?true:false">
				{{parentMess}}			
			</div>
		</div>
		<!--............各种类型按钮.........-->
		<div class="fixed-frame">			
			<div class="btn_con" v-if="buttonType==='出售中的商品'">
				<el-button 
					class="store-button1 float-l px-30" 
					@click="pulishExist">保存</el-button>
				<el-button 
					class="store-button2 float-l px-30" 
					@click="cancelPro">取 消</el-button>
				<el-button 
					class="store-button2 float-r px-30" 
					@click="offPro">下架商品</el-button>
			</div>
		</div>
	</div>
</template>
...
<script>
	...
	import {keepCategoryMess,createProduct,onoffBatch} from "@/api/commodity"
	...
	export default{
		components:{
			...
			"moreImg":()=>import("@/components/servicer/sellerCenter/moreImg"),
			"CommodityDetails":()=>import("@/components/servicer/sellerCenter/CommodityDetails")
		},
		data(){
			return{
				pulishForm:{
		...
		methods:{
			...
			pulishExist(pulishForm){//发布保存按钮在一起				
				if(this.title==="编辑商品"){
					...
					this._deleteIndex(this.pulishForm.shop_categorys);
